...
## 다른 모델로 m2/m19 교차검증 (같은 세션, 후속)

"로컬 20B 모델의 생성 신뢰성 문제"라는 진단이 정말 맞는지 — 코드가 아니라
모델 자체가 원인이라는 근거를 다른 모델로도 재현해서 확인했다. 과정에서
클라우드 judge/생성 엔드포인트 전환이 순탄치 않았다는 것도 같이 기록해둔다.

- **Groq(`openai/gpt-oss-20b`)**: 첫 시도는 문항 사이 텀 없이 20문항을
  연속 호출해 분당 토큰(TPM) 한도로 1문항도 못 마침. `langchain_core`의
  `InMemoryRateLimiter`를 `RAGChatbotV17.__init__`(`self.llm`/`self.intent_llm`)과
  `src/evaluation/llm_judge.py`(judge 클라이언트, 프로세스 전역 공유 인스턴스)
  양쪽에 `LLM_RATE_LIMIT_SECONDS` 환경변수로 붙여 호출 간 간격을 강제하는
  기능을 추가했다(기본 비활성 — 로컬 Ollama는 이런 한도가 없어 불필요하게
  느려지지 않도록). 다만 최종적으로는 **일일 토큰(TPD) 한도 자체**(200,000/일)가
  이미 그날의 반복 시도로 소진돼 있어 이 계정으로는 끝까지 못 갔다.
- **OpenAI 계정 1(`gpt-5.4-mini`)**: 결제수단 없는 계정 특유의 무료 체험
  할당(RPM 10, **RPD 50**)으로 m1~m8(32콜)까지는 성공 — 전부 Correctness
  4~5점. m9부터 하루 요청 한도 50건을 넘겨 실패. 재시도 간격을 29분
  기다려도 남은 사용량 숫자가 전혀 안 줄어들어(고정값), 롤링 윈도우가
  아니라 사실상 하루 단위 하드캡으로 판단하고 중단.
- **OpenAI 계정 2, 3**: 둘 다 `insufficient_quota`(크레딧 0, 결제수단
  없음)로 아예 시작도 못함 — 계정 1의 무료 체험 할당은 모든 신규 계정에
  자동으로 주어지는 게 아닌 것으로 보임.
- **Antigravity(로컬 설치)**: 검토했으나 채택 안 함 — Google 개인 계정
  OAuth 인증 기반 IDE 코딩 에이전트(`~/.gemini/antigravity-cli/`)라
  이 프로젝트의 `.env` API 키/`HWP_RAG_LLM_BASE_URL` 방식으로 바로
  스왑할 수 있는 범용 API가 아님.
- **Claude Haiku(서브에이전트, 최종 채택)**: 별도 API 키 없이, 이 세션
  자체의 Agent 도구로 Haiku 서브에이전트를 호출해 m19 하나만 검증.
  단순히 질문만 던진 게 아니라 `RFPAnswerGenerator.generate()`
  (`src/graph/nodes.py:439`)가 실제로 만드는 **system 프롬프트
  (`RFP_SYSTEM_PROMPT`) + 2단계 user 프롬프트(`EVIDENCE_REFINEMENT_PROMPT`
  → `ANSWER_GENERATION_FROM_EVIDENCE_PROMPT`)를 그대로(재구성한
  `_build_context()` 결과 포함) 복사해 두 번의 개별 호출로 재현했다.
  1단계(근거 압축) 호출에서 이미 두 공사 금액(368,467,000원 vs
  24,867,000원)을 정확히 추출했고, 2단계(최종 답변) 호출도 "건축이
  통신보다 기초금액이 더 큽니다"를 기대 답변과 정확히 일치하게 생성함.

**결론**: Ollama 전체 실행(`new20_v6`)에서 실패했던 m2/m19는 각각 다른
모델(OpenAI, Claude Haiku)로 **동일한 프로덕션 프롬프트 구조**를 재현했을 때
둘 다 정답을 냈다. 20문항 전부가 최소 한 개 이상의 모델에서 정답으로
교차검증됐고, retrieval은 20문항 전 구간에서 이미 recall@5=1.0으로
확인돼 있었으므로 — **이번 세션에서 다룬 리트리벌/리랭킹 버그 수정 범위는
