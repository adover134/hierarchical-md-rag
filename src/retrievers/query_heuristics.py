...
def is_implicit_follow_up_query(query: str) -> bool:
    ...
    if any(slot in q for slot in follow_up_slots):
        # "마감일은?"처럼 주어가 생략된 짧은 후속질문만 잡으려는 의도였는데, 길이
        # 제한이 없어서 "OO사업의 공사기간은 며칠인가요?"처럼 자체 주어(전체
        # 사업명)를 온전히 갖춘 완전한 질문도 "기간"/"예산" 같은 흔한 단어 하나로
        # 무조건 후속질문 취급되어, 이전 턴의 무관한 org_name을 그대로 물려받는
        # 문제가 있었다(실측 확인 — 5~10자짜리 진짜 생략형과 30자+ 완전한 질문
        # 사이에 뚜렷한 길이 격차가 있어 20자를 기준으로 구분).
        return len(q) <= 20
...
def looks_like_project_phrase(text: str) -> bool:
    ...
    project_markers = [
        ...
        "포털", "플랫폼", "조사", "연계", "기능",
        # "공사"는 "~공사"(한국전력공사 등) 기관명 접미사와 "타일공사"/"설치공사"처럼
        # 건설공사 자체를 뜻하는 일반명사 둘 다로 쓰인다. 이 코퍼스는 후자(입찰공고문의
        # 공사명)가 압도적으로 많고 org_registry에도 "~공사"로 끝나는 진짜 기관명이
        # 없어(직접 확인) 여기 포함해도 안전하다 — 없으면 정규식 기반 org 추출기가
        # "타일공사"를 기관명으로 오판했을 때 이 안전장치가 구제하지 못한다.
        "공사",
    ]
